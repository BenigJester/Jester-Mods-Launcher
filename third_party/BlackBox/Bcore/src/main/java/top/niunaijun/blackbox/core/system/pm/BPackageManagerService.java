...
public class BPackageManagerService extends IBPackageManagerService.Stub implements ISystemService {
    ...
    private InstallResult installPackageAsUserLocked(String file, InstallOption option, int userId) {
        ...
        try {
            ...
            // A FLAG_SYSTEM install clones an app that Android has already installed for this
            // physical device. Its native ABI may be supplied by splitSourceDirs instead of the
            // base APK, so validating base.apk alone can falsely reject Play Services/Play Store
            // and other App Bundle installs. Storage/import installs still require the normal ABI
            // validation because BlackBox has no host installation proving compatibility.
            if (!option.isFlag(InstallOption.FLAG_SYSTEM)) {
                ...
                Slog.i(TAG, "FLAG_SYSTEM clone: trusting host-installed ABI/splits for " + packageArchiveInfo.packageName);
            }
            PackageParser.Package aPackage = option.isFlag(InstallOption.FLAG_SYSTEM)
                    ? parserHostInstalledPackage(packageName, apkFile)
                    : parserApk(apkFile.getAbsolutePath());
            if (aPackage == null) {
    ...
    private static boolean isApksBundle(File file) {
        ...
        // A normal APK is itself a ZIP and may legitimately embed other APK files (Google Play
        // services Dynamite modules are a common example). Never reinterpret an explicit .apk as
        // an APKS archive just because its ZIP contains nested APK entries.
        if (lowerName.endsWith(".apk")) {
            return false;
        }
        // Fallback only for extension-less/renamed document-provider files. Detect by zip content.
    ...
    private static List<File> selectSplitApks(List<File> apks, File baseApk, boolean is64Bit) {
        ...
        // Preserve every feature/asset-pack split. Configuration splits are the only mutually
        // exclusive group and are selected by ABI, density, and language below.
        ArrayList<File> featureSplits = new ArrayList<>();
        for (File f : nonBase) {
        ...
        // Heuristic configuration selection: pick at most one ABI, one density, one language.
        File abiSplit = null;
        ...
        ArrayList<File> result = new ArrayList<>(featureSplits);
    ...
    /**
     * Host-cloned packages may be App Bundles whose exported Android components live in
     * split manifests. Parsing only base.apk copies splitSourceDirs into ApplicationInfo but
     * leaves BlackBox's component resolver unaware of services/activities declared by those
     * splits. Parse the installed APK cluster when possible so virtual resolution matches the
     * package Android actually installed on the device.
     */
    private PackageParser.Package parserHostInstalledPackage(String packageName, File fallbackBaseApk) {
        try {
            ApplicationInfo hostInfo = BlackBoxCore.getPackageManager().getApplicationInfo(packageName, 0);
            String[] splitSourceDirs = hostInfo != null ? hostInfo.splitSourceDirs : null;
            if (hostInfo != null && hostInfo.sourceDir != null
                    && splitSourceDirs != null && splitSourceDirs.length > 0) {
                File hostBaseApk = new File(hostInfo.sourceDir);
                File clusterDir = hostBaseApk.getParentFile();
                if (clusterDir != null && clusterDir.isDirectory()) {
                    PackageParser.Package clusterPackage = parserApk(clusterDir.getAbsolutePath());
                    if (clusterPackage != null && packageName.equals(clusterPackage.packageName)) {
                        Slog.i(TAG, "FLAG_SYSTEM cluster parsed: " + packageName
                                + " splits=" + splitSourceDirs.length
                                + " activities=" + (clusterPackage.activities != null ? clusterPackage.activities.size() : 0)
                                + " services=" + (clusterPackage.services != null ? clusterPackage.services.size() : 0)
                                + " receivers=" + (clusterPackage.receivers != null ? clusterPackage.receivers.size() : 0)
                                + " providers=" + (clusterPackage.providers != null ? clusterPackage.providers.size() : 0));
                        return clusterPackage;
                    }
                    Slog.w(TAG, "FLAG_SYSTEM cluster parse did not return expected package "
                            + packageName + "; falling back to base.apk");
                }
            }
        } catch (Throwable t) {
            Slog.w(TAG, "FLAG_SYSTEM cluster parse failed for " + packageName
                    + "; falling back to base.apk", t);
        }
        return parserApk(fallbackBaseApk.getAbsolutePath());
    }
