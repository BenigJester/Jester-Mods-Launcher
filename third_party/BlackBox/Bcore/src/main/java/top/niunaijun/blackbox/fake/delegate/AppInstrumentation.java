...
public final class AppInstrumentation extends BaseInstrumentationDelegate implements IInjectHook {
    ...
    private static Context ensureNonNullApplicationContext(Context context) {
        ...
        // Instrumentation.newApplication normally receives the framework's ContextImpl.
        // Keep that concrete object intact even though getApplicationContext() is still null
        // during Application.attach(). Native application shells (for example NetEase's
        // libnesec) legitimately access ContextImpl fields through JNI and abort under
        // CheckJNI when handed the anonymous ContextWrapper fallback below.
        if (isFrameworkContextImpl(context)) {
            return context;
        }
    ...
    private static boolean isFrameworkContextImpl(Context context) {
        Class<?> type = context.getClass();
        while (type != null) {
            if ("android.app.ContextImpl".equals(type.getName())) {
                return true;
            }
