...
public class IPackageManagerProxy extends BinderInvocationStub {
    ...
    private static boolean isGoogleCompatibilityPackage(String packageName) {
    ...
    private static Object invokeHostPackageManager(Object who, Method method, Object[] args) {
        try {
            Object[] hostArgs = args == null ? null : args.clone();
            if (hostArgs == null) return null;
            MethodParameterUtils.replaceUserIdIfNeeded(hostArgs, hostArgs.length - 1);
            return method.invoke(who, hostArgs);
        } catch (Throwable error) {
            Slog.w("GoogleSignInCompat", "Host package metadata fallback failed", error);
            return null;
        }
    }
    ...
    public static class GetPackageInfo extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            if ("com.android.vending".equals(packageName)) {
                // If the real Play Store has been cloned into this BlackBox user, prefer its
                // parsed virtual package record. Play-protected apps rely on the real virtual Play
                // package version/signing metadata; the historical fake record is only a fallback
                // for users that intentionally do not install a virtual Play stack.
                PackageInfo virtualVending = BlackBoxCore.getBPackageManager()
                        .getPackageInfo(packageName, flags, BlackBoxCore.getUserId());
                if (virtualVending != null) {
                    Slog.d(TAG, "GetPackageInfo: Using virtual Play Store info for user "
                            + BlackBoxCore.getUserId());
                    return virtualVending;
                }

                Object hostPlayStore = invokeHostPackageManager(who, method, args);
                if (hostPlayStore instanceof PackageInfo) {
                    Slog.i("GoogleSignInCompat", "Using host package metadata fallback for "
                            + packageName + " caller=" + BActivityThread.getAppPackageName());
                    return hostPlayStore;
                }

                PackageInfo fake = createFakeGooglePlayServicesPackageInfo();
                // Last-resort historical fallback for devices without a visible Play Store.
            ...
            if (isGoogleCompatibilityPackage(packageName)) {
                Object hostGoogle = invokeHostPackageManager(who, method, args);
                if (hostGoogle instanceof PackageInfo) {
                    Slog.i("GoogleSignInCompat", "Using host package metadata fallback for "
                            + packageName + " caller=" + BActivityThread.getAppPackageName());
                    return hostGoogle;
                }
    ...
    public static class GetPackageUid extends MethodHook {
        @Override
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            String packageName = args != null && args.length > 0 && args[0] instanceof String
                    ? (String) args[0] : null;
            if (packageName != null && BlackBoxCore.get().isInstalled(
                    packageName, BlackBoxCore.getUserId())) {
                ApplicationInfo virtualInfo = BlackBoxCore.getBPackageManager().getApplicationInfo(
                        packageName, 0, BlackBoxCore.getUserId());
                if (virtualInfo != null) {
                    // Inside the sandbox, callers must see the same virtual app id that
                    // Binder.getCallingUid() exposes. Returning the host UID here breaks
                    // Play licensing's UID/package consistency check.
                    return virtualInfo.uid;
                }
            }
            MethodParameterUtils.replaceUserIdIfNeeded(args, args.length - 1);
            return method.invoke(who, args);
    ...
    public static class GetApplicationInfo extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            if (isGoogleCompatibilityPackage(packageName)) {
                Object hostGoogle = invokeHostPackageManager(who, method, args);
                if (hostGoogle instanceof ApplicationInfo) {
                    Slog.i("GoogleSignInCompat", "Using host application metadata fallback for "
                            + packageName + " caller=" + BActivityThread.getAppPackageName());
                    return hostGoogle;
                }
            }
    ...
    public static class ResolveContentProvider extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            if (BlackBoxMediaContract.PUBLIC_AUTHORITY.equals(authority)) {
                Object[] privateMediaArgs = args.clone();
                privateMediaArgs[0] = BlackBoxCore.getHostPkg() + ".blackbox.MediaProvider";
                Object hostResult = invokeHostPackageManager(who, method, privateMediaArgs);
                if (hostResult instanceof ProviderInfo) {
                    ProviderInfo providerInfo = (ProviderInfo) hostResult;
            ...
            if (providerInfo == null) {
                // BlackBoxCore.getPackageManager() is backed by this proxy in some guest/OEM
                // combinations. Calling resolveContentProvider() through it re-enters this hook
                // until the guest dies with StackOverflowError. Query the saved original
                // IPackageManager object directly, just like the other host fallbacks above.
    ...
    @ProxyMethod("getComponentEnabledSetting")
    // Keep a distinct JVM name on Windows filesystems, which are case-insensitive.
    // The original fork also declares GetComponentEnabledSetting above.
    public static class GetComponentEnabledSettingLegacy extends MethodHook {
        @Override
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ComponentName componentName = (ComponentName) args[0];
