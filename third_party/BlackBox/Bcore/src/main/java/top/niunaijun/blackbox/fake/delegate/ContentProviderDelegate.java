...
public class ContentProviderDelegate {
    ...
    private static IInterface wrapProvider(IInterface provider, String auth) {
        return wrapProvider(provider, auth, resolveProviderCallerPackage(auth));
    }

    private static IInterface wrapProvider(IInterface provider, String auth, String callerPackage) {
        switch (auth) {
            case "media":
    ...
    public static void update(Object holder, String auth) {
        update(holder, auth, resolveProviderCallerPackage(auth));
    }

    /**
     * Wraps a real, host-owned provider without presenting the virtual package as its caller.
     * This is required for Storage Access Framework grants: Android grants the selected document
     * URI to the BlackBox host UID/package, not to the package that is virtualized inside it.
     */
    public static void updateHost(Object holder, String auth) {
        update(holder, auth, BlackBoxCore.getHostPkg());
    }

    private static void update(Object holder, String auth, String callerPackage) {
        ...
        if (iInterface instanceof Proxy)
            return;
        IInterface bContentProvider = wrapProvider(iInterface, auth, callerPackage);
        if (BuildCompat.isOreo()) {
