...
public class IActivityManagerProxy extends ClassInvocationStub {
    ...
    public static class GetContentProvider extends MethodHook {
        ...
        private static boolean isDocumentsProvider(ProviderInfo provider) {
            if (provider == null || TextUtils.isEmpty(provider.authority)) {
                return false;
            }
            try {
                Intent query = new Intent(DocumentsContract.PROVIDER_INTERFACE);
                for (ResolveInfo resolved : BlackBoxCore.getPackageManager()
                        .queryIntentContentProviders(query, PackageManager.MATCH_ALL)) {
                    ProviderInfo candidate = resolved == null ? null : resolved.providerInfo;
                    if (candidate == null || TextUtils.isEmpty(candidate.authority)) {
                        continue;
                    }
                    for (String authority : candidate.authority.split(";")) {
                        if (provider.authority.equals(authority)) {
                            return true;
                        }
                    }
                }
            } catch (Throwable error) {
                Slog.w(TAG, "Could not classify document provider " + provider.authority
                        + ": " + error.getMessage());
            }
            return false;
        }
        ...
        try {
            ...
            if (isPlayLicensingIntent(intent)) {
                String resolved = resolveInfo != null && resolveInfo.serviceInfo != null
                        ...
                        : "<host-pass-through>";
                Slog.i(TAG, "Play licensing bind: app=" + BActivityThread.getAppPackageName()
                        + " user=" + userId + " action=" + intent.getAction()
                        + " component=" + intent.getComponent() + " resolved=" + resolved);
            }
            if (isGoogleServiceIntent(intent)) {
                String resolved = resolveInfo != null && resolveInfo.serviceInfo != null
                        ? resolveInfo.serviceInfo.packageName + "/" + resolveInfo.serviceInfo.name
                        : "<unresolved>";
                Slog.i("GoogleSignInCompat", "GMS bind request app="
                        + BActivityThread.getAppPackageName() + " user=" + userId
                        + " action=" + intent.getAction() + " package=" + intent.getPackage()
                        + " component=" + intent.getComponent() + " resolved=" + resolved);
            }
    ...
    private static boolean isGoogleServiceIntent(Intent intent) {
    ...
    private static boolean isPlayLicensingIntent(Intent intent) {
        if (intent == null) return false;
        String action = intent.getAction();
        ComponentName component = intent.getComponent();
        if (action != null && action.toLowerCase(java.util.Locale.ROOT).contains("licens")) {
            return true;
        }
        return component != null && "com.android.vending".equals(component.getPackageName());
    }
    ...
    public static class SendIntentSender extends MethodHook {
        ...
        @Override
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            // PendingIntent activity targets are already rewritten by GetIntentSender to
            // BlackBox's ProxyPendingActivity. Do not swallow the later send request: the
            // host ActivityManager must receive it so the proxy Activity can unwrap and
            // launch the original virtual target (for example Google account selection).
            String ownerPackage = null;
            if (args != null && args.length > 0 && args[0] instanceof IInterface) {
                try {
                    ownerPackage = BlackBoxCore.getBActivityManager()
                            .getPackageForIntentSender(((IInterface) args[0]).asBinder());
                } catch (Throwable ignored) {
                }
            }
            Intent fillIn = null;
            ...
            Slog.i("GoogleSignInCompat", "Forwarding IntentSender owner=" + ownerPackage
                    + " app=" + BActivityThread.getAppPackageName()
                    + " fillIn=" + (fillIn == null ? "<none>" : fillIn.toUri(0)));
            return method.invoke(who, args);
        }
    }
