...
public class ContentProviderStub extends ClassInvocationStub implements BContentProvider {
    ...
    private void fixCallingIdentity(Object[] args) {
        if (args == null) return;
        String callerPackage = mAppPkg != null && !mAppPkg.isEmpty()
                ? mAppPkg : BlackBoxCore.getHostPkg();
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            if (arg != null && attributionSourceClass != null
                ...
                ContextCompat.fixAttributionSourceState(
                        arg, callerPackage, BlackBoxCore.getHostUid());
            } else if (arg instanceof String && hostPackage != null
                    && hostPackage.equals(arg)) {
                // Only rewrite Android's calling-package field. The previous implementation
                // replaced every String argument, corrupting values such as openFile's "w" mode,
                // query selections, MIME types, and provider method names.
                args[i] = callerPackage;
            }
        }
    }
