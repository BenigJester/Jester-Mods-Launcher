...
public class BActivityManagerService extends IBActivityManagerService.Stub implements ISystemService {
    ...
    public ComponentName startService(Intent intent, String resolvedType, boolean requireForeground, int userId) {
        ...
        // Do not hold the global ActiveServices monitor while resolving/starting a guest process.
        // Google Play services performs nested ActivityManager service calls from Application.onCreate();
        // serializing those calls behind one monitor can deadlock a caller that is simultaneously
        // starting Play Billing or another Google service. ActiveServices synchronizes its record
        // maps at the actual mutation points instead.
    ...
    public IBinder peekService(Intent intent, String resolvedType, int userId) throws RemoteException {
        UserSpace userSpace = getOrCreateSpaceLocked(userId);
        // peekService can make a remote call into the guest process. Never keep the global
        // ActiveServices monitor across that Binder hop; the guest may re-enter ActivityManager.
    ...
    public Intent bindService(Intent service, IBinder binder, String resolvedType, int userId) throws RemoteException {
        UserSpace userSpace = getOrCreateSpaceLocked(userId);
        // bindService may synchronously create a new virtual process. GMS recursively binds more
        // services while its Application is starting, so holding mActiveServices across process
        // creation creates a lock inversion with other callers (for example Play Billing on the
        // game's main thread). Keep only the record-map mutations synchronized in ActiveServices.
