...
void DexFileHook::init(JNIEnv *env) {
    // Android 16 changed ART's DexFile internals. Replacing this JNI entry on
    // API 36+ can leave ART with an invalid virtual-method mapping and abort at
    // a later checkpoint. Module DEX files are sealed read-only by the launcher,
    // so keep this legacy fallback only on Android 14 and 15.
    constexpr int kAndroidApi36 = 36;
    const int apiLevel = BoxCore::getApiLevel();
    if (apiLevel >= __ANDROID_API_U__ && apiLevel < kAndroidApi36) {
