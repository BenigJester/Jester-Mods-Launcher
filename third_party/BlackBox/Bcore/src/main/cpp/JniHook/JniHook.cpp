...
namespace {
constexpr int kAndroidApi36 = 36;

bool SupportsArtMethodMutation() {
    return HookEnv.api_level > 0 && HookEnv.api_level < kAndroidApi36;
}
}
...
JniHook::HookJniFun(JNIEnv *env, const char *class_name, const char *method_name, const char *sign,
    ...
    if (!SupportsArtMethodMutation() || HookEnv.art_method_native_offset == 0) {
        return;
    }
...
void JniHook::InitJniHook(JNIEnv *env, int api_level) {
    ...
    // Android 16 validates internal ArtMethod mappings while walking stacks
    // and visiting GC roots. RegisterNatives-based replacement of framework
    // methods can corrupt those mappings and cause a delayed ART abort from an
    // unrelated thread. Keep the supported native/libc hooks, but do not read
    // or mutate ART's private method layout on API 36+.
    if (!SupportsArtMethodMutation()) {
        ALOGD("JniHook: ART method replacement disabled on API %d", api_level);
        return;
    }
