...
int main(int argc, char *args[])
{
    ...
    try
    {
        ...
        if (inj_cfg.payload_dex.empty() != inj_cfg.payload_native.empty())
        {
            throw std::runtime_error("--payload-dex and --payload-native must be used together");
        }
    }
    ...
    if (!inj_cfg.payload_dex.empty())
    {
        KITTY_LOGI("payload_dex: %s", inj_cfg.payload_dex.c_str());
        KITTY_LOGI("payload_native: %s", inj_cfg.payload_native.c_str());
    }
    KITTY_LOGI("================================");
